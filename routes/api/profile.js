...
router.get('/user/:user_id', async (req, res) => {
    try {
      ...
      if (err.kind == 'ObjectId') {
        return res.status(400).json({ msg: 'Profile not founds' });
      }
      res.status(500).send('Server Error');
    }
  });
...
router.delete('/experience/:exp_id', auth, async (req, res) => {
    try {
        const profile = await Profile.findOne({ user: req.user.id });
        //Get removeIndex = profile.experience
...
router.delete('/education/:edu_id', auth, async (req, res) => {
    try {
        const profile = await Profile.findOne({ user: req.user.id });
        //Get removeIndex = profile.education
...
router.get('/github/:username', (req, res) => {
    try {
        const options = {
            uri: `https://api.github.com/users/${req.params.username}/repos?per_page=5&sort=createed:asc&client_id=${config.get('githubClientId')}&client_secret=${config.get('githubSecret')}`,
            method: 'GET',
